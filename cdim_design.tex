...
These detectors range in technology-readiness-level (TRL), but all are sufficiently developed to be considered for the 2020 Decadal and will be demonstrated on missions such as NEOCam, SPHEREx, and JWST\@. \red{Citation needed.}
...
Each detector \red{nominally} dissipates less than \SI{4}{\milli\watt}, for a total power dissipation of less than \SI{150}{\milli\watt} for the full array.
...
CDIM optics, instruments, and focal plane will be housed in a light-tight box \red{to minimize signal contamination from stray light}.
\red{The light-tight box is constructed of gold-plated aluminum.}
...
\red{A hybrid passive-active cryocooling scheme divides the heat dissipation loads among passive and active heat sources.
...
\red{At \Ltwo, solar radiation pressure presents itself as a constant torque on the spacecraft on the order of $10^{-5}$\si{\newton\meter}.}
...
\red{
  ...
  Images will be processed on-board CDIM using an algorithm demonstrated by SPHEREx.
}
...
\red{In order to survey the entire sky, the cells must be able to adjust to account for different incident angles to the sun.}
...
\red{The CDIM spacecraft bus will not feature novel technology.} Rather it will leverage high TRL or off-the-shelf components.
